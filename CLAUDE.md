...
- ✅ 済み：**YouTube公開前の総点検（2026-05-26）**。全ページをパソコン幅・スマホ幅で実機確認、全リンク（公式ページ）の生存確認、横はみ出しゼロを測定で確認。
  見つかった4点を修正：①日本時間のズレ対策（カウントダウンを時間帯に左右されず正確に）②カードに「くわしく見る→」を常時表示（スマホでも押せると伝わる）
  ③石川県大会を毎年開催あつかいに（開催日が過ぎても消えない）④自動お片付けで毎年開催の大会を消さないように。ビルド・lintともにエラーゼロ
- 🟡 進行中：（なし。運用フェーズ）
- 🔜 次の一歩：本番へ公開（GitHub main へ push で自動公開）／申請が来たら確認・承認（下記「申請の確認方法」）／全国主要大会をさらに追加
