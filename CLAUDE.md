...
- ✅ 済み：本気版リリース。正直な本物データ（主要大会20＝日本将棋連盟・LPSA・JT・イオン・天童・高槻・石川/千葉/東京の県団体戦等／うち女子大会6・実在教室3＝LPSA女子教室含む）、
  女子専用ページ /joshi（女子の大会＋女子も安心の教室をピンクで集約）、
  トップの地方ナビ RegionPicker（地方→都道府県で北海道〜沖縄をさがせる）、
...
- ✅ 済み：**見た目とスマホ対応の大改良**。スマホは画面下に大きな指で押せるメニュー（ホーム/大会/教室/女子/登録）、
  パソコンは上のメニュー。トップに虹色ライン・水玉＆パステルの背景・カード上ふちの色帯。
  検索（さがす画面）は①学年→②女子→③地域の3ステップ説明つき・学年に絵文字・「いまの条件」表示・「ぜんぶ消す」ボタンを追加
- ✅ 済み：**YouTube公開前の総点検（2026-05-26）**。全ページをパソコン幅・スマホ幅で実機確認、全リンク生存確認、横はみ出しゼロを測定で確認。4点修正（日本時間のズレ／カードに「くわしく見る→」常時表示／石川県大会を毎年開催あつかい／自動お片付けで毎年開催を消さない）
- ✅ 済み：**仕上げ6点を追加（2026-05-26）**。
  A=「はじめての子・おうちの方へ」案内ページ /hajimete（弱くてもOK・持ち物・親の付き添い・費用・当日の流れのQ&A。ヘッダー/フッター/トップから導線）、
  D=大会・教室の詳細に共有ボタン（LINE/X/URLコピー）、
  F=詳細に「情報がちがう・古い」報告リンク（/api/report→submissionsにkind='report'保存。承認では公開されない安全策つき・実機テストずみ）、
  E=SNS共有画像を日本語化（opengraph-image。Google Fontsから必要文字だけTTF取得。失敗時は英語版に自動フォールバック）、
  B+C=実在教室を3つ追加（ヤマダこども将棋教室=全国・連盟公認／柏将棋センター=千葉柏・石田九段／ねこまど=東京新宿）＋東京の教室URLを子供将棋スクール専用ページに更新＋小学生名人戦を「公文杯」正式リンクに＋教室ページに外部検索リンク（連盟教室一覧・いつつ）。教室は3→6件
- ✅ 済み：**さらに4点を追加（2026-05-26）**。
  G=クリックできる日本地図（src/components/JapanMap.tsx・タイル配置・地方ごと色分け→/chiikiへ。トップに設置）、
  H=都道府県ごとの専用ページ /chiiki/[ローマ字]（例 /chiiki/tokyo。県固有のタイトル・説明・パンくず・その県の大会＋教室＋全国教室を表示。47県分。サイトマップにも追加）、
  I=地方の実在教室を5つ追加（札幌将棋センター/仙台・将棋サロン彩/名古屋・坂江将棋教室/広島将棋センター※女子も/福岡将棋会館）。教室は6→11件で北海道〜九州を網羅、
  J=トップに「最終チェック日」表示＋検索エンジン向け構造化データ（WebSite＝layout、BreadcrumbList＝/chiiki）。
  ローマ字スラッグは src/lib/prefectures.ts の PREF_SLUG/SLUG_TO_PREF。地方ナビ(RegionPicker)のリンク先も /chiiki に統一
- 🟡 進行中：（なし。運用フェーズ）
- 🔜 次の一歩：申請が来たら確認・承認（下記「申請の確認方法」）／教室・大会をさらに追加（本物のみ）
- ⚠️ 運用メモ：seed.ts のデータを増やしたら、本番反映には自動更新の手動実行が必要 → `curl "https://shogi-taikai-navi.vercel.app/api/cron/refresh?key=<CRON_SECRET>"`（サイトはDBから表示するため）
